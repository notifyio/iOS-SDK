...
Download the SDK
Add the SDK to your project by dragging the [file a] and the [file b] you downloaded into your Xcode project folder target.  Make sure the "Copy items to destination's group folder" checkbox is checked
https://www.parse.com/images/apps/quickstart/demo_ios_existing1.png?5506573

Add the dependencies. Click on Targets -> Your app name -> then the ‘Build Phases’ tab. Expand ‘Link Binary with Libraries’ as shown: 

https://www.parse.com/images/apps/quickstart/demo_ios_existing2.png?5506573

Click the + button on the bottom left of the ‘Link Binary with Libraries’ section and add the following libraries:
...
SystemConfiguration.framework
     ...
     4.  Add the -objc linker flag. 
     5. Must disable bitcode in build settings
...
We’ve designed our SDK to allow you to request push permissions at any time in your application's lifecycle.  We recommend that you request push permissions only when you believe the user will opt-in them.  This is usually after the user understands the value of push notifications.  Some best practices include, after the user becomes engaged, or when the user triggers a feature that request notifications.  Please contact us to discuss optimizing your opt-in follow.  
...
Viewed for a user viewing an item. This is the most standard event type. 
Engaged this action is used to indicate that the user has engaged with the item.  For a video this might mean the user has vided more than 50% of it.  For content, it can mean the user has scrolled through more than 50% of the page.  
Ended indicates that the user has completed viewing or reading the item.  For example it would indicate that the user has reached the end of the video.  
Purchased when the user has completed the purchase of an item.  If the user purchases multiple items you’ll have to make an individual call for each purchase. 
Added To Cart after a user adds an item to their cart.  
...
Actions are always associated with items that we have ingested or will be ingest in the future.  The item parameter(itemID) specifies the unique identifier for the item and should match the unique_id specified in the ingestion API.
...
Attributes include, but are not limited to email, age, gender, telephone number and other demographic information.  User attributes can be set using the  addUserAttribute API. 
...
3.14 The user id of the Currently Logged in User
...
3.15 How to get your push token
...
3.17 notificationSubscription Management 
We provide advanced subscription management for users allowing them to opt-in or opt-out of individual types of notification (like product recommendations or trending content)   

Each user’s subscription state for each user are synced to our backend.  To get a list of notification types and the user’s current subscription state you register handlers with the registerSubscriptionListHandlers.  The use of these handlers is explained below. 
...
Notify?.registerSubscriptionListHandlers(OnSuccess, 
onError: OnError, onUpdate: OnUpdate)
...
NotifyConfig.plist options
“automaticSetupEnabled”
If enabled the SDK will automatically request push permissions on launch.  (Do not include this in the documents) 
What is and do we respect “productionLogLevel”?
You can see the options on common/globals.h
Calls use things like lerror, lwarn, linfo 
Review NSLog see if it should be one of the other error levels
What is “detectProvisioningMode”?
Handles the auto sandbox detection for push certificate selection. 
Don’t include in developer docs. 

3.9 Track events
Events give you the ability to track import milestones during execution of your application.  Even tracking will be used for future capabilities.  

Following example code demonstrates how to track events. 
Swift
Glance.getInstance().trackEvent(event)
Glance.getInstance().trackEvent(event, withDoubleValue: value)
Glance.getInstance().trackEvent(event, withStringValue: value)


Objective C
[self.glance trackEvent:event];
[self.glance trackEvent:event withDoubleValue:value];
[self.glance trackEvent:event withStringValue:value];
